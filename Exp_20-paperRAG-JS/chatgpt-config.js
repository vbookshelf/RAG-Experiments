...
async function makeApiRequest(my_message) {
	  ...
	  try {
		...
		for (var i = 0; i < num_in_context; i++) {
			...
			var text = response_json['data']['Get']['ARXIV_100SAMPLE_VDB'][i]['abstract'];
			// Remove the title portion from the string
			// Split on '{title}' and choose the second list item
			text = text.split('{title}')[1];
			
			var title_text = response_json['data']['Get']['ARXIV_100SAMPLE_VDB'][i]['title'];
			var arxiv_id = response_json['data']['Get']['ARXIV_100SAMPLE_VDB'][i]['arxiv_id'];
			
			var pdf_link = 'https://arxiv.org/pdf/' + arxiv_id;
			
			text = `${title_text}<br>
			<a class="w3-text-blue" href=${pdf_link}" target=”_blank”>
			${pdf_link}</a>  
			<p>${text}</p>
			`;
		...
		You will be provided with a list of titles and abstracts for research papers: 
		${context}
		
		Write a one sentence summary of each abstract at the level of a high scholl student.
		`;
		...
		var message_list;
		system_setup_message = "You are a helpful assistant.";
		message_list = [{"role": "system", "content": system_setup_message}];
