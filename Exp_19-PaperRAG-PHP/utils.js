...
function show_paper_info_panel(response_json, num_results_to_display) {
	...
	for (var i = 0; i < num_results_to_display; i++) {
		...
		var cat_text = response_json['data']['Get'][vectordb_name][i]['cat_text'];
        ...
        div.innerHTML = `
            <div>
			<p>Title: ${title_text}<br>
			Categories: ${cat_text}</p>
                <p>Abstract: ${abstract}<br>
                <a class="w3-text-blue" href="${pdf_link}" target="_blank">${pdf_link}</a></p>
            </div>
			`;
	    ...
	    context_list.push(abstract); // Append the value of i to the array
