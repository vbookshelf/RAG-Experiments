...
var system_setup_message = `
		...
		If the text from the database doesn't contain the answer, ask the user for more information.	
		...
		Always answer all questions in the context of the OHS Act.
		`;
...
async function makeApiRequest(my_message) {
	  ...
	  try {
		...
		// The model has output function arguments
			...
			// First lets check the message history and delete any data
			// that came from the database
			// Define the key to check
			//const keyToCheck = 'role';
			//const valueToExclude = 'function';
			
			// Filter out dictionaries where the specified key has the value 'function'
			//message_list = message_list.filter(dict => dict[keyToCheck] !== valueToExclude);
